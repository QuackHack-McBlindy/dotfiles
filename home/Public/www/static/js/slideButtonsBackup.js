...
function displayDeviceItems(devices, listElement, itemClass) {
    ...
    devices.forEach(device => {
        ...
        listItem.addEventListener('click', () => {
            fetch(/set_default_device/${device.ip}, { method: 'POST' })  // Use device IP in the request
                .then(() => {
                    alert(${device.name} is now the default device.);
                    fetchDeviceList();  // Refresh the device list to update the default
                })
    ...
    listItem.addEventListener('mousedown', () => {
        longPressTimeout = setTimeout(() => {
            ...
            const apiUrl = listItem.classList.contains('directory')
                ? /api/playback/append?media_type=${mediaType}
                : /api/playback/append_file?media_type=${mediaType};
