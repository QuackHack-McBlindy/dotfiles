...
in
{
  imports = [
    ...
    (mkRenamedOptionModule [ "services" "caddy" "ca" ] [ "services" "caddyduckdns" "acmeCA" ])
    (mkRenamedOptionModule [ "services" "caddy" "config" ] [ "services" "caddyduckdns" "extraConfig" ])
  ];

  # interface
  options.services.caddyduckdns = {
    ...
    configFile = mkOption {
      type = types.path;
      default = configFile;
      defaultText = "A Caddyfile automatically generated by values from services.caddyduckdns.*";
      example = literalExpression ''
    ...
    enableReload = mkOption {
      ...
      description = ''
        ...
        to a non-infinite value in {option}`services.caddyduckdns.globalConfig`
    ...
    settings = mkOption {
      ...
      description = ''
        ...
        Please use a Caddyfile via {option}`services.caddyduckdns.configFile`, {option}`services.caddyduckdns.virtualHosts` or
        {option}`services.caddyduckdns.extraConfig` with {option}`services.caddyduckdns.globalConfig` instead.
        :::

        ::: {.note}
        Takes presence over most `services.caddyduckdns.*` options, such as {option}`services.caddyduckdns.configFile` and {option}`services.caddyduckdns.virtualHosts`, if specified.
        :::
      '';
    };
  };
      ...
      [
        {
          ...
          message = "To specify an adapter other than 'caddyfile' please provide your own configuration via `services.caddyduckdns.configFile`";
        }
      ]
      ++ map (
        name:
        mkCertOwnershipAssertion {
          ...
          services = [ config.systemd.services.caddyduckdns ];
        }
      ) vhostCertNames;

    services.caddyduckdns.globalConfig = ''
    ...
    systemd.services.caddyduckdns = {
      ...
      serviceConfig =
        ...
        in
        {
          ExecStart = [
            ""
            ''./result/bin/caddy run ${runOptions} ${optionalString cfg.resume "--resume"}''
          ];
          ExecReload = [
            ""
      ...
      let
        certCfg = map (
          ...
          nameValuePair certName {
            ...
            reloadServices = [ "caddyduckdns.service" ];
          }
        ) vhostCertNames;
      in
      listToAttrs certCfg;
