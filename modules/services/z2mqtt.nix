...
let
  # Helper function to define the devices list
  devices = {
  ...
  # Generate the YAML structure for devices
  devicesYaml = format.generate "devices.yaml" devices;
...
in {
  ...
  services.zigbee2mqtt = {
    ...
    settings = {
      ...
      availability = true;
      permit_join = false;
      # This section doesn't need to include the devices in Nix anymore
      devices = [];  # Will be generated in devices.yaml instead.
      groups = {
        "1" = {
  ...
  systemd.services.zigbee2mqtt = {
    preStart = ''
      # Write the generated YAML to devices.yaml
      echo "${devicesYaml}" > ${config.services.zigbee2mqtt.dataDir}/devices.yaml
    '';
  };
}
