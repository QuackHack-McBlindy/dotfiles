...
EOF
    ...
    code = ''
      ...
      #mqtt_pub -t "zigduck/crypto/btc/price" -m "{\"current_price\": \"$BTC_PRICE\", \"24h_change\": \"$BTC_24H\", \"7d_change\": \"$BTC_7D\"}"
      FORMATTED_24H=$(printf "%.1f" "$BTC_24H")
      FORMATTED_7D=$(printf "%.1f" "$BTC_7D")
      mqtt_pub -t "zigduck/crypto/btc/price" -m "{\"current_price\": $BTC_PRICE, \"24h_change\": $FORMATTED_24H, \"7d_change\": $FORMATTED_7D}"
