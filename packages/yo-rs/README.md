...
Minimal multi-client microphone audio streaming with wake-word detection and transcription with optional pass the transcribed text to an externall shell command.
Just start the server+client with defaults to run locally.
...
```nix
      services.yo-rs = {
        server = {
          enable = true;
          host = "0.0.0.0:12345";
          execCommand = "yo do"; # Will exec: yo do "transcribed text"
