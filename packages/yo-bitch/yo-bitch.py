...
async def transcribe_audio(audio: UploadFile = File(...)):
 ...
 #       send_to_hassil(transcription)
        ...
        if "bitch" in transcription.lower():
            command = await parse_voice_command(transcription)
            await execute_yo_intent(command)
            return {"transcription": transcription}
        
        await speak("Jag hörde inte wake-ordet 'bitch'")
        return {"transcription": "No wake word detected"}

    except Exception as e:
        await speak("Fel vid transkribering", urgent=True)
